"""Golden tests for the option-set ConceptMaps: the shape, the identity, and parity with SUSHI's own output."""
...
def test_the_emitted_map_matches_what_sushi_compiles_from_the_same_instance() -> None:
    ...
    assert _document([_BIRTH_TYPE], _NAME_SOURCE) == json.loads(_SUSHI_GOLDEN.read_text(encoding="utf-8"))
...
def test_two_groups_resolve_both_dhis2_identifiers_of_every_concept() -> None:
    ...
    document = _document([_BIRTH_TYPE], _NAME_SOURCE)
...
def test_code_source_maps_the_dhis2_code_concept_back_to_its_uid() -> None:
    ...
    document = _document([_BIRTH_TYPE], GenerateConfig(naming=NamingConfig(source="name"), concept_code_source="code"))
    assert _rows(_group(document, "http://dhis2.org/fhir/id/option")) == [
        ("NB", "kRRUtYaGett"),
        ("CS", "EBE0c8sZazS"),
        ("GVcG84DTFOB", "GVcG84DTFOB"),
...
def test_a_set_whose_options_carry_no_code_emits_the_uid_group_alone() -> None:
    ...
    uncoded = OptionSetIn(
        uid="Ys9d8f7g6h5", name="Uncoded", options=[OptionIn(uid="Op1aaaaaaaa", name="One", sort_order=1)]
    )
    document = _document([uncoded], _NAME_SOURCE)
...
def test_a_code_that_is_not_a_valid_fhir_code_is_left_out_of_the_code_group() -> None:
    ...
    document = _document([hostile], _NAME_SOURCE)
...
def test_an_option_set_without_options_emits_no_map() -> None:
    ...
    assert _maps([empty], _NAME_SOURCE) == []
    assert build_option_set_concept_map_artifacts([empty], _NAME_SOURCE, _CANONICAL, ig_status="draft") == []
...
def test_a_skipped_option_is_absent_from_every_group() -> None:
    ...
    document = _document([collided], GenerateConfig(naming=NamingConfig(source="name"), concept_code_source="code"))
...
def test_the_map_takes_its_identity_from_the_same_plan_the_pair_does() -> None:
    ...
    identity = option_set_identities([_BIRTH_TYPE], _NAME_SOURCE).identities[0]
    document = _document([_BIRTH_TYPE], _NAME_SOURCE)
...
def test_the_uid_source_default_names_the_map_after_the_option_set_uid() -> None:
    """Default naming source is the UID, mixed case included, exactly as the pair's ids are."""
...
def test_publication_state_follows_the_ig_status() -> None:
    ...
    draft = json.loads(_maps([_BIRTH_TYPE], _NAME_SOURCE)[0].model_dump_json(exclude_none=True, by_alias=True))
    active = json.loads(
        _maps([_BIRTH_TYPE], _NAME_SOURCE, "active")[0].model_dump_json(exclude_none=True, by_alias=True)
    )
...
def test_the_target_systems_follow_the_configured_identifier_base() -> None:
    ...
    config = GenerateConfig(naming=NamingConfig(source="name"), identifier_system_base="https://example.org/dhis2")
...
def test_the_map_urls_follow_the_ig_canonical() -> None:
    ...
    concept_map = build_option_set_concept_maps(
        [_BIRTH_TYPE], _NAME_SOURCE, "https://ig.example/fhir", ig_status="draft"
    )[0]
...
def test_the_artifacts_land_in_their_own_directory_as_indented_json() -> None:
    ...
    artifacts = build_option_set_concept_map_artifacts([_BIRTH_TYPE], _NAME_SOURCE, _CANONICAL, ig_status="draft")
...
def test_one_map_per_option_set_in_the_plans_emission_order() -> None:
    ...
    other = OptionSetIn(uid="Ab1b2c3d4e5", name="Apgar", options=[OptionIn(uid="Op1aaaaaaaa", name="One")])
    artifacts = build_option_set_concept_map_artifacts(
        [_BIRTH_TYPE, other], _NAME_SOURCE, _CANONICAL, ig_status="draft"
    )
    assert [artifact.relative_path for artifact in artifacts] == [
