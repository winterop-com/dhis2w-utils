...
def test_makefile_serves_the_ig_off_disk_and_straight_from_the_instance() -> None:
    ...
    makefile = _by_path()["Makefile"]
    assert "serve:  ## Serve the compiled IG as a FHIR endpoint, with the capture UI at / (run generate + sushi first)" in makefile
    assert "\t$(D2W) fhir serve --ui\n" in makefile
    assert "serve-live:  ## Serve straight from the DHIS2 instance, with the capture UI at / - no compile needed" in makefile
