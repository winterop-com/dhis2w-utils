"""Tests that the R4 models read the real SUSHI output this package generates, key for key.

The models themselves live in `dhis2w_fhir_engine.r4.resources` and their shape is asserted in that
package's `tests/test_r4_resources.py`. What is asserted here is the pairing nobody else can assert:
that the compiled implementation guide in `tests/data/r4/` - the JSON SUSHI produced from the FSH
this package emitted - validates and re-emits unchanged through the facade capture code imports.
"""
