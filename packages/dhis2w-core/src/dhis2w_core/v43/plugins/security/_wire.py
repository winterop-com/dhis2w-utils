"""Version-specific wire extraction for the v43 security plugin (2FA + last login + route auth + tokens).

v43 removed every admin-readable per-user 2FA field from the User resource
...
The generated v43 `ApiToken` carries `type` as the `ApiTokenType` enum and `createdBy` as a `UserDto`
...
from dhis2w_client.generated.v43.oas import ApiToken, Dhis2OAuth2Client
...
def tokens_from_raw(raw: list[Any]) -> list[TokenView]:
    """Validate each raw /api/apiToken record into the v43 generated ApiToken and project a TokenView.

    The generated v43 `ApiToken.type` is the `ApiTokenType` enum; it is normalised to a plain str here so
...
def oauth2_clients(raw: dict[str, Any]) -> list[OAuth2ClientView]:
    """Project each v43 `/api/oAuth2Clients` record from the `oAuth2Clients` envelope into an OAuth2ClientView.

    v43 returns the clients under the `oAuth2Clients` key, each with the `clientId` identifier and the
