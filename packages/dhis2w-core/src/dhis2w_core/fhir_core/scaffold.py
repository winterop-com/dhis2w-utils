...
from dhis2w_core.fhir_core.models import InitOptions, ScaffoldFile
...
terminology = false                 # also emit DHIS2OrgUnitCS/VS with level/parent properties
"""
...
.PHONY: help setup generate sushi build clean clean-all
...
def build_scaffold_files(options: InitOptions) -> list[ScaffoldFile]:
    ...
    return [
        ScaffoldFile(
            relative_path="fhir.toml",
            content=_FHIR_TOML_TEMPLATE.format(
                ig_id=options.ig_id,
