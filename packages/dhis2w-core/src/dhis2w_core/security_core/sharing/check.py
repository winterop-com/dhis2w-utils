...
def evaluate_sharing(graph: SharingGraph) -> CheckOutcome:
    ...
    for object_node in graph.objects:
        for finding in _object_findings(object_node):
            log.record(finding)
    return log.result()


def _object_findings(object_node: ObjectNode) -> list[AuditFinding]:
    ...
    if object_node.external:
        findings.append(_external_finding(object_node))
    public = _public_finding(object_node)
    if public is not None:
        findings.append(public)
    return findings


def _public_finding(object_node: ObjectNode) -> AuditFinding | None:
    """The one public-exposure finding for an object, keyed off the shared precedence kind."""
    kind = public_exposure_kind(object_node)
    if kind is ExposureKind.PUBLIC_WRITE_DATA:
        return _public_write_data_finding(object_node)
    if kind is ExposureKind.PUBLIC_WRITE_METADATA:
        return _public_write_metadata_finding(object_node)
    if object_node.external:
        return None
    if kind is ExposureKind.SQL_VIEW_READ:
        return _sql_view_read_finding(object_node)
    if kind is ExposureKind.PUBLIC_DATA_READ:
        return _public_data_read_finding(object_node)
    return None


def _external_finding(object_node: ObjectNode) -> AuditFinding:
    ...
    return AuditFinding(
        ...
        detail=(
            f"{object_node.type} '{object_node.name}' has external access enabled; it is reachable anonymously, "
            "with no login. Anything its sharing exposes is readable by anyone on the network."
        ),
        subject=object_node.name,
        group_key="sharing-external",
        evidence=_evidence(object_node),
        control="sharing-external",
    )


def _public_write_data_finding(object_node: ObjectNode) -> AuditFinding:
    ...
    return AuditFinding(
        ...
        detail=(
            f"{object_node.type} '{object_node.name}' grants write to every authenticated user "
            f"(public {_axes(object_node)}). On a data-bearing type this lets any logged-in account alter "
            "its definition or its data."
        ),
        subject=object_node.name,
...
def _public_write_metadata_finding(object_node: ObjectNode) -> AuditFinding:
    ...
    return AuditFinding(
        ...
        detail=(
            f"{object_node.type} '{object_node.name}' grants metadata write to every authenticated user (public "
            f"{_axes(object_node)}); any logged-in account can change it."
        ),
        subject=object_node.name,
...
def _sql_view_read_finding(object_node: ObjectNode) -> AuditFinding:
    ...
    return AuditFinding(
        ...
        detail=(
            f"SQL view '{object_node.name}' is readable by every authenticated user; any logged-in account can "
...
def _public_data_read_finding(object_node: ObjectNode) -> AuditFinding:
    ...
    return AuditFinding(
        ...
        detail=(
            f"{object_node.type} '{object_node.name}' grants data read to every authenticated user "
            f"(public {_axes(object_node)}); its data is visible to any logged-in account, subject to "
            "org-unit scope."
        ),
        subject=object_node.name,
...
def _axes(object_node: ObjectNode) -> str:
    """Render an object's decoded public access as a compact `meta rw / data r-` summary."""
    meta = f"{'r' if object_node.public.meta_read else '-'}{'w' if object_node.public.meta_write else '-'}"
    data = f"{'r' if object_node.public.data_read else '-'}{'w' if object_node.public.data_write else '-'}"
    return f"meta {meta} / data {data}"


def _evidence(object_node: ObjectNode) -> dict[str, str]:
    """Render-only evidence bag for a sharing finding: the object's uid, type, and public access."""
    return {
        "uid": object_node.uid,
        "type": object_node.type,
        "public": _axes(object_node),
        "external": "true" if object_node.external else "false",
    }
