...
from dhis2w_core.v42.plugins.fhir import plugin
...
async def test_registers_three_tools() -> None:
    """The plugin exposes init plus the two generate tools."""
    async with Client(_server()) as client:
        names = {tool.name for tool in await client.list_tools()}
    assert names == {"fhir_init", "fhir_generate_option_sets", "fhir_generate_org_units"}
