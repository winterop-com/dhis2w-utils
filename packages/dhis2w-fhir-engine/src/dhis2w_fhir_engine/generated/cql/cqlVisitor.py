# ANTLR emits this file from the grammar in dhis2w_fhir_engine/grammars/. It is machine output and
# is excluded from every linter: ruff by extend-exclude in the member pyproject, mypy and pyright by
# the directives below, since both read the workspace-root config rather than a per-member one.
# ruff: noqa
# mypy: ignore-errors
# pyright: reportAttributeAccessIssue=false, reportArgumentType=false, reportOptionalMemberAccess=false
# pyright: reportWildcardImportFromLibrary=false, reportMissingImports=false, reportMissingParameterType=false
# pyright: reportIncompatibleMethodOverride=false, reportUnnecessaryComparison=false, reportUnusedImport=false
