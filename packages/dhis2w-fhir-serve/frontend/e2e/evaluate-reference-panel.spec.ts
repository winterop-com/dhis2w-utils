...
test('stops naming an example once the source it loaded has been typed over', async ({ page }) => {
    ...
    const row = page.getByTestId('evaluate-examples').getByRole('button', { name: 'The given names on a Patient' })
    await row.click()
    ...
    // What is in the box is a reader's own expression, and the panel must not say otherwise.
