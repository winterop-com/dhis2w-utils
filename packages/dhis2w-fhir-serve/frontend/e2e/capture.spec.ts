...
test('the receipt route is deep-linkable and shows the raw resource on demand', async ({
    ...
    await expect(page.getByText('Generated by test data, seed 43')).toBeVisible()
...
test.describe('filling a form in the browser', () => {
    test('opens a form, fills it with test data, submits, and finds the receipt', async ({
        ...
        // NAMED, NOT COUNTED. A clerk who files six forms in a morning needs to know which receipt
        // this one became - the id the create interaction states in `Location` - and the toast
        // carries the way straight to it.
        const accepted = page.locator('[data-sonner-toast]').filter({
            hasText: 'The server accepted this submission',
        })
        await expect(accepted.getByText(/^Stored as receipt [0-9a-f]{32}\.$/)).toBeVisible()
        await expect(accepted.getByRole('button', { name: 'Open', exact: true })).toBeVisible()
...
test.describe('a form whose data set reports per attribute option combo', () => {
    test('asks for the combo, refuses to submit without one, and puts it on the receipt', async ({
        ...
        await expect(page.getByText('Choose an attribute option combination before submitting')).toBeVisible()
...
test.describe('a form whose assignment narrows where it may be reported from', () => {
    ...
    test('offers only the assigned units, narrows on search, and puts the chosen one on the receipt', async ({
        ...
        await expect(
            page.getByText(
                "A capture outside the form's assigned organisation units is refused when it reaches this DHIS2 instance.",
            ),
        ).toBeVisible()
