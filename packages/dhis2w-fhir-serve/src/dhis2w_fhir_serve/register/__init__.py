...
Five modules split the work. `index` reads the published guide once, at startup, for the facts a
...
or searchable, which attributes each type's forms ask, and the names a program or an organisation
unit is published under. `surface` narrows that by `[serve.tracked_entities]` into what this run
answers. `wire` is the DHIS2 read, and holds the empirical contract the search obeys. `projection`
turns one tracked entity into the resource its type is registered as. `filtering` is the register's
value filter - `d2-attribute={attributeUid}|{value}`, which asks which records hold a value where
`identifier` asks which record is somebody.
"""
