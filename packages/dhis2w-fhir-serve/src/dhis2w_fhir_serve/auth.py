"""Who the facade serves: the three postures, the check they run, and the identity a capture is stamped with.

THREE POSTURES, ONE DEPENDENCY. `[serve] auth` picks between them and `[serve] auth_scope` picks how
...
what the name is reserved for and `docs/fhir/301-serving.md` says the same to a deployer.
"""
...
#: Where the auth state and the established identity are held, on the app and on the request.
...
class RequestIdentity(BaseModel):
    ...
    """The DHIS2 username, under the `dhis2` posture. None under `token`, which names no person."""
...
def challenge_for(posture: ServeAuth) -> str:
...
async def require_authenticated(request: Request) -> None:
    ...
    if presented == "":
        raise UnauthenticatedError(
            f"this server takes no request without an `Authorization` header; {_expected(settings.auth)}",
            challenge_for(settings.auth),
        )
...
def auth_state(request: Request) -> AuthState:
    ...
    if held is not None:
        return held
    state = AuthState(tokens=read_serve_tokens())
...
def _expected(posture: ServeAuth) -> str:
...
def preflight_auth(
    *,
    ...
    Three refusals, each about a run rather than about a request, which is why they are here and not
    ...
    if not stated and not _is_loopback(host):
        raise ServeAuthConfigurationError(
            ...
            f'to take a static bearer token out of `{SERVE_TOKENS_VARIABLE}`, or `auth = "dhis2"` to have '
            "every caller present the DHIS2 credentials this facade checks against the instance. "
