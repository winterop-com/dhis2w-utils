...
The repo lives at `winterop-com/dhis2w-utils`; PyPI ships the nine publishable members under the `dhis2w-*` prefix. Not affiliated with DHIS2.
...
| [`dhis2w-fhir`](https://pypi.org/project/dhis2w-fhir/) | `uv add dhis2w-fhir` | FHIR IG generation: `init` scaffolds a SUSHI project as a uv project with a pinned toolchain; `generate foundation / option-sets / questionnaires / examples / org-units / pages / all` emits FSH and narrative from DHIS2 metadata, including the capture contract (QuestionnaireResponse profiles + a D2CaptureServer CapabilityStatement); `validate` checks an instance's codes for FHIR-safety with md/csv/pdf reports. Mounts `d2w fhir` via the plugin entry point. |
...
| `d2w fhir` | FHIR IG generation (via `dhis2w-fhir`) — scaffold a SUSHI project as a pinned uv project (`fhir init`), generate the seven targets (`fhir generate foundation / option-sets / questionnaires / examples / org-units / pages / all`) covering identifier systems, the D2Period / D2FormType / D2AttributeValue extensions, the capture contract, terminology, forms with example responses, the org-unit registry carrying each unit's DHIS2 attribute values, and the narrative pages, check codes for FHIR-safety with md/csv/pdf reports (`fhir validate`) |
