...
| `d2w fhir` | FHIR IG generation (via `dhis2w-fhir`) — scaffold a SUSHI project as a pinned uv project (`fhir init`, with `--refresh` to bring an existing project's scaffold up to date), generate the seven targets (`fhir generate foundation / option-sets / categories / questionnaires / examples / org-units / pages / all`) covering identifier systems, the D2Period / D2FormType / D2AttributeValue extensions, the capture contract, option-set and category terminology, forms with example responses, the org-unit registry carrying each unit's DHIS2 attribute values, and the narrative pages, check codes for FHIR-safety with md/csv/pdf reports (`fhir validate`), write a synthetic load set (`fhir generate load`), and serve the compiled or live IG as a FHIR read + capture facade (`fhir serve`, via the `[serve]` extra) |
