#!/usr/bin/env bash
# POST /facade/evaluate and POST /$evaluate — the evaluation engine answering over a served guide, from curl.
...
#
#   POST /facade/evaluate  this project's own JSON - typed results, and a parse failure's line and column
...
evaluate_fhirpath() {
    curl -s -X POST "${BASE}/facade/evaluate" -H 'Content-Type: application/json' -d @- <<JSON
...
echo "a bad expression: HTTP $(curl -s -o parse.json -w '%{http_code}' \
      -X POST "${BASE}/facade/evaluate" -H 'Content-Type: application/json' \
...
#    same question asked at /facade/evaluate keeps "matched nothing" as a stated row with an empty value.
...
echo "  the same at /facade/evaluate: $(evaluate_fhirpath 'Questionnaire.publisher' | jq -c '.results[0]')"
