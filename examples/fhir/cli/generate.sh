...
# The exit code is the whole point of the command, so read it rather than let `set -e` take
# the script down: the seeded Sierra Leone fixture carries names FHIR cannot template
# ("<5 y"), so a run against it exits 1 and that is the gate working.
d2w fhir validate || echo "validate found errors - the CI gate did its job"
