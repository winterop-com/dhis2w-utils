...
| [`evaluate.sh`](cli/evaluate.sh) | `POST /facade/evaluate` and `POST /$evaluate` over a served guide, from curl - the OperationDefinition the contract lives in, FHIRPath over a stored form, a parse failure answered 200, and the `Parameters` a CQL library comes back as | yes |
...
| [`serve_record.sh`](cli/serve_record.sh) | `GET /facade/tracked-entities/{uid}/events` - one entity's own record as the responses its stage forms describe, one event on its own, and the `events` dial that withholds it | yes |
| [`serve_data_set_responses.sh`](cli/serve_data_set_responses.sh) | `GET /facade/data-sets/{uid}/responses` - what DHIS2 holds for one form, one organisation unit, and the periods a request names, each as the response its data set's published form describes; the bounds it refuses a read without, the period count it answers at once, and the `responses` dial that withholds the values while the forms stay published | yes |
