#!/usr/bin/env bash
# `d2w profile pat create` — provision a Personal Access Token as an admin.
...
d2w profile pat create --admin-user admin --description "demo token"
...
FRESH_PAT=$(d2w profile pat create --admin-user admin --description "script-captured" -q)
...
# DHIS2_PAT=$FRESH_PAT d2w profile add smoketest --url $DHIS2_URL --auth pat
