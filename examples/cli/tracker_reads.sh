...
# Run via `uv run bash examples/cli/tracker_reads.sh` so `d2w` resolves.
...
# List tracked entities by type. The seeded fixture ships a "Person"
...
d2w data tracker list "Person" --org-unit "$ORG_UNIT_UID" --page-size 5

# Fetch one entity by UID (pipes the first UID from the list above).
TE_UID=$(d2w --json data tracker list "Person" --org-unit "$ORG_UNIT_UID" --page-size 1 | jq -r '.[0].trackedEntity // empty')
if [ -n "$TE_UID" ]; then
