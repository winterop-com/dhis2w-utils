...
async def main() -> None:
    ...
    # No `version=`, so the client detects the major from /api/system/info and binds that tree.
    # Pass `version=Dhis2.V42` (or V41 / V43) to pin one instead — the client then refuses to
    # connect to a server of a different major rather than round-trip renamed fields wrong.
    async with Dhis2Client(base_url, auth=build_auth()) as client:
