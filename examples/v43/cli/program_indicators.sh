#!/usr/bin/env bash
# ProgramIndicator authoring surface under `d2w metadata`:
#
#   d2w metadata program-indicators         /api/programIndicators
#   d2w metadata program-indicator-groups   /api/programIndicatorGroups
#
...
d2w metadata list programIndicators --filter program.id:eq:"$CHILD_PROGRAM" --page-size 3 | head -10 || true
d2w metadata list programIndicatorGroups | head -10 || true
...
d2w metadata program-indicators validate-expression "#{$CHILD_PROGRAM.$BCG_DE}"

# ---------------------------------------------------------------------------
# Create PI + group, link, clean up.
# ---------------------------------------------------------------------------

PI_OUT=$(d2w --json metadata program-indicators create \
...
GROUP_OUT=$(d2w --json metadata program-indicator-groups create \
...
d2w metadata program-indicator-groups get "$GROUP_UID"

# Rename the PI (partial update).
d2w metadata program-indicators rename "$PI_UID" --short-name "ExPIv2"

# ---------------------------------------------------------------------------
# Cleanup
# ---------------------------------------------------------------------------

d2w metadata program-indicator-groups remove-members "$GROUP_UID" --program-indicator "$PI_UID"
d2w metadata program-indicator-groups delete "$GROUP_UID" --yes
d2w metadata program-indicators delete "$PI_UID" --yes
