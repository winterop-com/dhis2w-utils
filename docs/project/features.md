...
d2w fhir            FHIR IG generation (SUSHI/FSH + pre-built JSON, package dhis2w-fhir)
  ...
  generate              All seven targets in one run, off a single pass over the
                        ...
                        the terminal (--details prints them inline instead)
  ...
  validate              FHIR-safety of the instance's codes (sweep + two deep
                        ...
                        The terminal is a status view: the summary, a rollup row
                        per (severity, category), every error individually, and
                        one line naming the report file; --details lists every
