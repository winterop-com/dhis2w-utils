...
| Enrollment and incident dates | When an enrollment began and when the event it follows occurred | `CARRIED` | `D2EnrolledAt` (dateTime 1..1) and `D2IncidentAt` (dateTime 0..1) on the registration response, the second gated by `displayIncidentDate` | capture, forward |
| `displayIncidentDate` | Whether the programme asks for an incident date at all | `CARRIED` | Read in the projection and used to decide whether `D2IncidentAt` is emitted | capture |
