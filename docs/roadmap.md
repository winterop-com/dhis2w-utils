...
- **Missing authoring verbs: `optionSets` + `userGroups` `create`/`delete`** (only reachable via `metadata import` today). FEATURE. The generated accessors DO expose `.create(item)` / `.delete(uid)` (resources.py: OptionSet ~9455/9505, UserGroup ~15806/15856), but `.create()` returns a raw import-summary dict, so the service must create-then-fetch (extract `response.uid`, re-`get`) to return a typed model per the no-dict rule. Needs: service create/delete (metadata for optionSet via `options` sub-app; the `user_group` plugin for userGroup), CLI `create`/`delete` verbs, MCP, tests, and examples×3 + docs per the repo rules.
- **Inline tracker delete** (`data tracker event/enrollment/entity delete <uid>`). FEATURE. `push_tracker` with `importStrategy=DELETE` exists; a thin command builds `{"<kind>": [{"<idField>": uid}]}` and pushes (verify the minimal-bundle DELETE shape first — the observed working bundle carried program/stage/orgUnit). Async (returns a TrackerJob, not a confirmation). Needs commands×3 + tests + examples.
