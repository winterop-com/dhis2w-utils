...
- ~~**Help-text polish for analytics/data reads**~~ Shipped: filled the empty `--option` helps (tracker list/event-list, analytics start/end/ou-mode), expanded `--dim` (dx:/pe:/ou: + events `<stage>.<de>` no-dx: rule), and added runnable examples to analytics query/events + aggregate get docstrings. v41/v42/v43.
