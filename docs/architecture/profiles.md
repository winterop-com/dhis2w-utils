...
When the field is omitted, the CLI consults the `DHIS2_VERSION` env var (`41` / `42` / `43` → `v41` / `v42` / `v43`); when that's also unset, it defaults to the canonical v42 tree. A `version` pin wins over `DHIS2_VERSION`. `make verify-examples` follows the same `resolve_startup_version()` chain, so a v41-pinned profile runs the `examples/v41/...` tree automatically; on an unpinned profile, `make verify-examples DHIS2_VERSION=43` drives the v43 tree without editing the profile.
