...
- **PR 2 (credential probe + revised guardrail contract): implemented and verified on `feat/security-audit-scanner` (2026-06-18).** Added the version-invariant `security_core/credentials.py` (the `ProbeOutcome`/`CredentialProbeResult` models, `classify_probe_status`, and `evaluate_credential_probe` — a 200 is a CRITICAL "default credentials accepted", a 401 passes finding-free, anything else is INFO inconclusive), extended `guardrails.py` with `DEFAULT_PROBE_USERNAME`/`DEFAULT_PROBE_PASSWORD` (admin/district, confirmed against the DHIS2 source `DefaultAdminUserPopulator`), `CREDENTIAL_PROBE_PATHS = {"/api/me"}`, `MAX_PROBE_ATTEMPTS = 1`, and flipped `REPORT_GUARDRAIL_NOTE` to state the one-login-attempt posture. The per-tree `audit.py` runs the probe as a step: it reads `keyLockMultipleFailedLogins` first and prints a prominent stderr warning when lockout is active, then issues exactly one HTTP Basic `GET /api/me` via a throwaway client (never the operator's profile, never retried, transport error -> DEGRADED). `credential-probe` is now in `IMPLEMENTED_CHECK_KEYS`, runs by default, and `--credential-probe/--no-credential-probe` toggles it across v41/v42/v43. Examples (3 trees), `features.md`, and the regenerated `cli-reference.md` are in sync. Gate: ruff + mypy + pyright clean; 85 security tests (a new `test_security_credential_probe.py` asserting the one-GET/Basic-admin-district/no-retry guardrail per tree, the verdict shaping, the lockout warning, and default-on wiring) and the full 546-test dhis2w-core suite green. Resolved a plan open question: `keyLockMultipleFailedLogins` is API-readable on all three versions (it is already in the shipped `SecuritySettings` projection), so no `--dhis-conf` fallback or BUGS.md entry was needed.
- **PR 3 (roles + hygiene): implemented and verified on `feat/security-audit-scanner` (2026-06-18), in two commits.** Roles (`security_core/roles.py` -> `RoleAudit` / `build_role_audit` / `evaluate_roles`): reads `/api/userRoles` with `users~size` member counts and flags ALL-granting roles CRITICAL (HIGH when unused) and dangerous-category roles HIGH/MEDIUM; version-uniform across the three trees. Hygiene (`security_core/hygiene.py`): per-user flags over privileged accounts (never-logged-in HIGH, stale beyond `--stale-days` default 90 MEDIUM, disabled-but-privileged MEDIUM, missing email WARN), seed-`admin`-enabled MEDIUM, and an active/disabled inventory INFO. The 2FA source diverges and is isolated in a new per-tree `_wire.py`: v41 reads `twoFactorEnabled` from `/api/users`; v42/v43 read the superuser-only `/api/users/twoFactor/summary` (`privileged.withAllAuthorityMissing2FA`) added on master by [dhis2-core#23925](https://github.com/dhis2/dhis2-core/pull/23925), degrading cleanly to a note when the endpoint 404s (not yet backported) or 403s (auditing account is not a superuser). The new `--two-factor-detail` flag names each superuser lacking 2FA via the per-user list endpoint. The allowlist gained `/api/users/twoFactor` (+ `/summary`); BUGS.md #47 records the 2FA gap as fixed-in-master-pending-backport. Gate: ruff + mypy + pyright clean; the full dhis2w-core suite green (new `test_security_roles.py`, `test_security_hygiene.py`). Resolved the plan's open 2FA-field question: the field was removed from the User resource on v42 and replaced by the audit endpoints, so v42/v43 cannot read it inline (BUGS.md #47); the v42/v43 2FA path is unit-tested against the documented endpoint shape because the backport is not yet on the play servers. Deferred to a follow-up: suspicious-username and account-expiry-in-past sub-signals.
- **PR 4 (version): implemented and verified on `feat/security-audit-scanner` (2026-06-18).** New `security_core/releases.py` (the `releases.dhis2.org/v1/versions/stable.json` feed parsed into `ReleaseFeed`/`ReleaseLine`, TTL-cached, the audit's one external egress) and `security_core/versions.py` (`parse_dhis2_version` over `2.{line}.{patch}[.hotfix][-SNAPSHOT]`, plus `evaluate_version`). The check flags EOL lines (feed `supported=false`, or below the v41 floor) HIGH, below-advisory-floor HIGH, and behind-on-the-latest-patch-OR-hotfix-within-the-line WARN; a stripped version is INFO. Because DHIS2 maintains a rolling support window (latest ~3 lines supported at once), a supported-but-not-latest line is a healthy state, so a newer line is reported only as an INFO note, never a problem. The within-line currency check compares the full `(patch, hotfix)` tuple against the feed's `latestPatchVersion`/`latestHotfixVersion`, so a missing security hotfix like `2.42.5.1` is caught even when the patch matches (`ReleaseLine.latest_hotfix`; old EOL lines null both counts, coerced to 0). Decision locked with the user: the advisory patch floor is a **static curated `ADVISORY_PATCH_FLOOR` table** in `versions.py` (v41>=8.2, v42>=5.1, v43>=0.1), seeded from the DHIS2 GitHub advisories (`GHSA-pwmg-mvjw-4m23`, `GHSA-3fr2-wvqx-cmr5`, `GHSA-6785-hj47-c27h`) with a `ADVISORY_PATCH_FLOOR_REVIEWED` date, kept in-repo so the single-egress guardrail stays intact (no live api.github.com calls). The feed fetch degrades to a note when releases.dhis2.org is unreachable, leaving the static advisory floor still active. Version-uniform across the three trees (no `_wire.py`). Gate: ruff + mypy + pyright clean; full dhis2w-core suite green (new `test_security_version.py`, including release-feed egress containment). Resolved the plan's open questions on the advisory-floor source/maintenance. The three play servers (2.41.9 / 2.42.6 / 2.43.1-SNAPSHOT) all sit above the floor, so no false advisory finding.
- **HTML report redesign (out-of-band, not one of the numbered PRs): implemented and verified on `feat/security-audit-scanner` (2026-06-19).** The `html` format moved from a single self-contained table to a data-driven bundle: a fixed template (`report.dc.html`), runtime (`support.js`), and logo ship as `dhis2w-core` package data and are copied into each run folder beside a per-scan `report-data.js` (`window.__REPORT__`). Per-account findings fold into collapsible groups via a new `group_key` on `AuditFinding` (folding keys on `(group_key, severity)` so the hero scorecard always equals the runtime's per-severity sum of group counts); other findings render one row each. Renderers share a uniform `emit()` write step (`SingleFileRenderer` for md/txt/csv, a bundle emitter for html), driving both `finalize` and `report` re-render. New `security_core/report/view.py`; full design + decisions in `DHIS2_Security_Report_Redesign_PLAN.md`. Gate: ruff + mypy + pyright clean; full suite green; an adversarial multi-dimensional review pass was run and its confirmed findings (producer/view test seam, rerender coverage, scorecard-invariant hardening, greenfield CHANGELOG wording) were all addressed.
- **PR 5 (apps): implemented and verified on `feat/security-audit-scanner` (2026-06-19).** New `security_core/apps.py` (`InstalledApp` / `HubApp` models + `evaluate_apps`): side-loaded apps installed outside the App Hub (no `app_hub_id`, and not `bundled` / `core_app`) are HIGH untrusted-frontend-code findings folded under one `group_key`; hub-backed apps behind the App Hub's latest semver are MEDIUM update-available findings; configured custom JavaScript / CSS (`keyCustomJs` / `keyCustomCss`, read via the allowlisted `/api/systemSettings`) are HIGH persistent-XSS findings. The per-tree `_run_apps` maps the client's typed `apps.list_apps()` / `apps.hub_list()` accessors into the version-invariant models and reads `/api/apps` + `/api/appHub` (both already in `GET_ALLOWLIST`); a hub fetch error degrades to a note and skips only the update comparison, so an App Hub outage never reads as "all untrusted" while side-loaded and custom-code signals still run. `apps` is in `IMPLEMENTED_CHECK_KEYS` and runs by default across v41/v42/v43. Gate: ruff + mypy + pyright clean; new `test_security_apps.py` (version-invariant `evaluate_apps` coverage plus per-tree `_run_apps` wiring against a mock client); full dhis2w-core suite green. No new allowlist paths and no BUGS.md divergence: the generated `App` exposes `app_hub_id` / `bundled` / `core_app` on all three trees, so the check is version-uniform.

- **PR 6 (guest): implemented and verified on `feat/security-audit-scanner` (2026-06-19).** New `security_core/guest.py` (`AnonymousResult` / `GuestProbeTarget` models + `evaluate_guest`): an unauthenticated probe of `/api/users` (CRITICAL), `/api/userRoles` (HIGH), and `/api/me` (HIGH) flags any that return 200 without a login; a non-null `/api/configuration/selfRegistrationRole` is a HIGH "self-registration enabled"; `keyAccountRecovery` is an INFO note. The per-tree `_run_guest` runs the anonymous probe through a throwaway httpx client (no credentials, never retried -- an unauthenticated read is not a login attempt, so no lockout risk), then reads the self-registration role and account-recovery setting with the operator's client. **Deviation from the plan, resolved against the DHIS2 source:** the plan listed `/api/systemSettings` among the anonymous-leak targets, but DHIS2 serves its non-confidential settings anonymously by design (the login page needs them) and filters confidential keys server-side, so an anonymous 200 there is expected -- probing it would be a false positive on every instance, so it is excluded. The self-registration-captcha signal (`keySelfRegistrationNoRecaptcha`) already lives in the `settings` check, so the guest check does not duplicate it. No new allowlist paths (the four probe endpoints plus `/api/configuration/selfRegistrationRole` are already in `GET_ALLOWLIST`); the guardrail contract docstring gains an explicit note that the guest check issues unauthenticated reads. `guest` is in `IMPLEMENTED_CHECK_KEYS` and runs by default across v41/v42/v43. Gate: ruff + mypy + pyright clean; new `test_security_guest.py` (version-invariant `evaluate_guest` coverage plus per-tree `_run_guest` wiring with respx-intercepted anonymous probes); full dhis2w-core suite green.

- **PR 7 (sharing): implemented and verified on `feat/security-audit-scanner`.** New `security_core/sharing/` subpackage built around one versioned `SharingGraph` (the locked `model.py` contract: `ObjectNode` / `UserNode` / `UserGroupNode` / `RoleNode` nodes, `ShareEdge` / `MembershipEdge` / `RoleGrantEdge` / `ManageEdge` edges, decoded `AccessBits`) that both the findings and the explorer project from -- the graph is built once, never fetched or derived twice. `builder.py` (`build_sharing_graph`) assembles the graph from version-neutral fetched records; `focus.py` (`resolve_focus_types`) resolves the shareable type set from `/api/schemas` (`shareable` / `dataShareable`) rather than a hardcode; `check.py` (`evaluate_sharing`) reduces the graph into findings -- externalAccess HIGH (reachable anonymously), public-write to data-bearing objects HIGH, public-write metadata, and broad public-read of data or SQL views MEDIUM, with the read finding suppressed when external or public-write already dominates. The scan is paged and GET-only, capped by `--max-objects` (default 5000) with a loud truncation note. The opt-in `--sharing-graph` (alias `--visualize`) flag additionally writes the static, offline d3 explorer bundle (`sharing-explorer.html` + per-scan `sharing-data.js` + vendored `d3.min.js` + logo, mirroring the report HTML bundle pattern): `effective.py` (`compute_effective_access`) computes the transitive effective-access closure with provenance, `view.py` (`build_sharing_view`) + `explorer.py` (`ExplorerRenderer`) project it into the bundle. Explorer modes: object tree, exposure triage, by-principal and by-role/authority pivots, force-directed graph, and matrix heatmap; the report's sharing section deep-links the explorer. `sharing` is in `IMPLEMENTED_CHECK_KEYS` and runs by default across v41/v42/v43. Landed across `2d4bfc2c` (graph + findings + core explorer), `c15ed612` (force graph + matrix heatmap), and `85f00328` (design refresh); full design in `SHARING-EXPLORER-PLAN.md`. Gate: ruff + mypy + pyright clean; new `test_security_sharing.py`; full dhis2w-core suite green.

- **PR 8 (posture extras): implemented and verified on `feat/security-audit-scanner` (2026-06-25), in three sub-PRs.** Completes the 14-check catalog. Full design in `SECURITY-POSTURE-EXTRAS-PLAN.md`.
  - **8a (transport + settings extensions)** (`1a7da685`): new `security_core/transport.py` reads TLS posture and security headers from one `/api/system/info` response via `client.get_response` (non-TLS base URL HIGH, missing HSTS/CSP MEDIUM, missing anti-framing/nosniff WARN, server-version disclosure WARN; no new allowlist). The `settings` reducer gains four verdicts: self-grant authorities HIGH, permissive CORS wildcard MEDIUM (read from the new `/api/configuration/corsWhitelist` allowlist entry, since `keyCorsWhitelist` was removed in v2.31), email-unconfigured-while-recovery-or-verification-on WARN, and a global-2FA-not-enforced INFO note (INFO, not MEDIUM, so it never floors a clean instance's worst severity). BUGS.md #49, #50.
  - **8b (auth-methods + tokens + routes)** (`f8c19125`, `e9b22859`, `649f4655`): the three wire-divergent checks. `routes` (`security_core/routes.py`) flags Route API destinations on private/internal/cloud-metadata hosts (SSRF), normalizing non-canonical IP encodings (decimal/hex/octal/trailing-dot) and validating each route individually so an unknown auth type never drops the others; the v41 undiscriminated vs v42/v43 discriminated auth union is isolated in `_wire.py` (BUGS.md #14). `tokens` (`security_core/tokens.py`) inventories PATs (`/api/apiToken`, caller-scoped) and flags non-expiring / no-IP-allowlist HIGH with an account-scope caveat; the v41 Literal vs v42/v43 enum `ApiToken` shape is isolated in `_wire.py` (BUGS.md #51). `auth-methods` (`security_core/auth_methods.py`) inventories OIDC providers (`/api/loginConfig`) + OAuth2 clients (`/api/oAuth2Clients`), flagging broad grants and loose redirect URIs (loopback exempt per RFC 8252); the v41-only `OAuth2Client` vs v42/v43-only `Dhis2OAuth2Client` gap is bridged by a hand-rolled secret-free `OAuth2ClientView` (BUGS.md #52, cross-ref #39). Allowlist gains `/api/routes`, `/api/apiToken`, `/api/loginConfig`, `/api/oAuth2Clients`. No view carries a secret.
  - **8c (audit-config)** (`52ac0f1f`): `security_core/audit_config.py` + `dhisconf.py`. DHIS2 audit posture is dhis.conf-only (no API exposes it), so the API-first path reports an INFO note (never "auditing is off"); an optional `--dhis-conf PATH` (env `DHIS2_CONF_LOCATION`) parses a local copy and flags auditing-disabled / both-sinks-off / narrower-than-default scope MEDIUM. Redaction is enforced by construction (`RedactedSecret(key, is_set)` has no value field; a negative test renders every format and asserts a sentinel password never appears). Matrix semantics match `AuditMatrixConfigurer`: an absent/empty matrix applies the forensic default {CREATE, UPDATE, DELETE, SECURITY}, so a default install is audited and is not flagged. BUGS.md #53, #54.
  - Every sub-PR was adversarially reviewed; the reviews caught and fixed (with live verification) a routes SSRF encoding bypass and an auth-block crash, the settings 2FA-severity floor, the audit-config default-install false positive, a three-tree except-drift, and over-broad exception catches. Gate per sub-PR: ruff + mypy + pyright clean; the full dhis2w-core suite green (3646 tests after 8c). Only PR 9 (the cheap MCP read surface) remains before the scanner is release-complete.

- **PR 9 (MCP read surface): implemented and verified on `feat/security-audit-scanner` (2026-06-25, commit `a14b6635`).** The security plugin gains an `mcp.py` across v41/v42/v43 exposing three cheap, single-request, read-only tools mirroring the CLI: `security_settings` -> `SecuritySettings`, `security_authorities` -> `AccountAuthorities`, and `security_version` -> a new frozen `VersionPosture` (added to `security_core/versions.py`, reusing `evaluate_version`). `register_mcp` is wired into the plugin `__init__` mirroring the `route` plugin, so the FastMCP server auto-discovers the tools. The long-running `audit` and the active probes (credential-probe, guest) stay CLI-only -- no audit tool is registered. `security_version` reads the on-connect-cached `/api/system/info` and evaluates with `feed=None`, so the tool makes NO external release-feed egress (a single DHIS2 request; the feed-based behind-latest-patch refinement stays audit-only). All three tools carry `readOnlyHint=True` so they are correctly allowed under the opt-in read-only MCP mode (`DHIS2_MCP_READONLY`). Gate: ruff + mypy + pyright clean; new `test_security_mcp.py` (per-tree FastMCP `Client` coverage incl. the no-audit-leak and no-external-egress assertions) plus a read-only-visibility test; full suite green. This completes the 9-PR scanner plan; only the deferred js-x-ray app-bundle static analysis (see `SECURITY-CHECKS-FROM-AUDITOR-APP-PLAN.md` section 4) remains.
