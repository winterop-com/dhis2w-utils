...
- **New `dhis2w-fhir` package: FHIR IG generation from DHIS2 metadata** (2026-08-01). A new publishable workspace member mounting the version-neutral `fhir` plugin through the `dhis2.plugins` entry point (the first in-repo user of the external-plugin mechanism; `dhis2w-cli` and `dhis2w-mcp` depend on it so `d2w fhir` is present by default). `d2w fhir init` scaffolds a complete dockerized SUSHI IG project (minimal `fhir.toml` + exhaustive `fhir.toml.example`, `sushi-config.yaml`, `ig.ini` on `fhir2.base.template` per the 2026-03 FHIR npm-dependency security notice, `aliases.fsh`, Makefile with an `upgrade` target that pulls the latest SUSHI + IG publisher, Dockerfile). `d2w fhir generate option-sets|org-units|all` emits FSH into `ig/input/fsh/`: option sets as CodeSystem/ValueSet pairs where every concept carries both DHIS2 identifiers (UID as concept code by default with the DHIS2 code as a `dhis2-code` property; `concept_code_source = "code"` swaps them, gated by the R4 `code` prose constraint with loud UID fallback; ids bounded to the 64-character R4 id limit with UID-suffixed truncation), and organisation units as Organization instances with `partOf` hierarchy in one file per level plus Location instances for Point geometry (GeoJSON `[lng, lat]` swapped) and an optional org-unit CodeSystem/ValueSet with level/parent/dhis2-code properties. Artifact naming is configurable via `[generate.naming]` (`prefix`/`option_set`/`org_unit` tokens, short context-readable `D2`/`OS`/`OU` defaults, empty prefix allowed - profiles keep a token since FSH cannot shadow a core resource name). `d2w fhir validate` (read-only MCP `fhir_validate`) checks an instance's codes and names against the R4 primitives and exits 1 on errors for CI preflight. Generate output states the resolved profile and its provenance (flag/env, fhir.toml, or default chain). Generated files carry a header line; regeneration deletes only header-bearing files. Verified end-to-end against the play 2.42 Sierra Leone demo: 171 option sets + 1332 org units compile clean under SUSHI (0 errors, 0 warnings). Examples in `examples/v{41,42,43}/{cli,mcp}/`, architecture page `docs/architecture/fhir-plugin.md`. See `packages/dhis2w-fhir/`.
