...
- **BUGS.md remediation program planned in PLAN CENTER** (2026-07-13). Scaffolded `.pc/` with master plan `bugs-remediation` and 10 grouped child plans covering all 73 BUGS.md entries as repo-side remediations (not upstream dhis2-core fixes). Handoff checklists live under `.pc/tasks/` (`README.md` + `01`–`10`). Groups: cleanup fixed-upstream workarounds, version pin unblocks (#43/#44), codegen/OAS, auth/OAuth2, data/analytics, metadata import, client/CLI UX, query filters, security posture, upstream track-only. Run `pc validate` / `pc serve` to browse; execute later by flipping one plan to `ongoing`.
