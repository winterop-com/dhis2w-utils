...
- **FHIR plugin: SUSHI IG scaffolding + FSH generation from DHIS2 metadata** (2026-07-31). New `fhir` plugin across all three version trees. `d2w fhir init` scaffolds a complete dockerized SUSHI IG project (`fhir.toml`, `ig/sushi-config.yaml`, `ig/ig.ini`, `aliases.fsh`, Makefile, Dockerfile); `d2w fhir generate option-sets|org-units|all` emits FSH into `ig/input/fsh/`: option sets as CodeSystem/ValueSet pairs where every concept carries both DHIS2 identifiers (UID as concept code by default with the DHIS2 code as a `dhis2-code` property; `concept_code_source = "code"` swaps them, gated by a FHIR-code validity check with loud UID fallback; artifact ids are bounded to FHIR's 64-character id limit with UID-suffixed truncation for long names), and organisation units as `DHIS2Organization` instances with `partOf` hierarchy in one file per level, plus `DHIS2Location` instances for Point geometry (GeoJSON `[lng, lat]` swapped into latitude/longitude) and an optional `DHIS2OrgUnitCS`/`VS` representation with level/parent/dhis2-code concept properties (`terminology = true`). Generation config is a committed `fhir.toml` discovered by walking up from the working directory (the `.dhis2/profiles.toml` idiom); it may pin a d2w `profile`, with explicit `-p`/`DHIS2_PROFILE` taking precedence. Generated files carry a header line and regeneration deletes only header-bearing files, so hand-authored FSH is never touched. Version-neutral emission/scaffolding lives once in `dhis2w_core/fhir_core/` (the `security_core` split); the thin per-tree plugin adds `fhir_init`, `fhir_generate_option_sets`, and `fhir_generate_org_units` MCP tools (write-classified, refused under `DHIS2_MCP_READONLY=1`). Examples in `examples/v{41,42,43}/{cli,mcp}/`, architecture page `docs/architecture/fhir-plugin.md`, 65 new tests (golden FSH, config round-trip, CLI/MCP, three-tree parity). See `packages/dhis2w-core/src/dhis2w_core/fhir_core/`.
