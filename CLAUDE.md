...
13. **Makefile drives every workflow.** Core targets: `make install / lint / test / test-slow / coverage / docs / docs-serve / docs-build / build / publish-client / clean / clean-artifacts`, plus target families for the local DHIS2 stack (`dhis2-*`), codegen (`dhis2-codegen-*`), and model benchmarking (`bench-*`) — `make help` lists them all. CI calls make targets, not raw commands.
